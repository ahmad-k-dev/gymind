...
export function LoginScreen({ navigation }: { navigation: Nav }) {
  ...
  async function submit() {
    if (!email || !pass) { Alert.alert('Required', 'Fill in all fields'); return; }
    try { await login(email, pass); } catch {}
  }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          <View style={s.logoRow}>
            <View style={s.logo}><Text style={{ fontSize: 28 }}>🏋️</Text></View>
            <Text style={s.brand}>GYMIND</Text>
          </View>
          <Text style={s.h1}>Welcome back</Text>
          <Text style={s.sub}>Sign in to your account</Text>
          {!!error && <View style={s.errBox}><Text style={s.errTxt}>{error}</Text><TouchableOpacity onPress={clearErr}><Text style={s.errX}>✕</Text></TouchableOpacity></View>}
          <View style={s.form}>
            <Text style={s.label}>EMAIL</Text>
            <TextInput style={s.input} value={email} onChangeText={setEmail} placeholder="you@example.com" placeholderTextColor={C.muted} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} />
            <Text style={s.label}>PASSWORD</Text>
            <View style={s.passRow}>
              <TextInput style={s.passInput} value={pass} onChangeText={setPass} placeholder="••••••••" placeholderTextColor={C.muted} secureTextEntry={!showPass} autoCapitalize="none" />
              <TouchableOpacity onPress={() => setShowPass(v => !v)}><Text style={{ fontSize: 18 }}>{showPass ? '🙈' : '👁'}</Text></TouchableOpacity>
            </View>
